...
| `--no-link` | do NOT symlink gitignored dirs into the worktree |
...
The worktree is usable immediately — `node_modules`, `.venv`, and friends are
symlinked from the main checkout, so an agent can install, build, and test
without a five-minute cold start:

```bash
cd "$(uv run scripts/worktree.py root)/.worktrees/hotfix-auth"
git status --porcelain      # clean — the symlinks are excluded (see below)
npm test                    # works: node_modules resolves to the main checkout's
```
