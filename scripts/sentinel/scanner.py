...
class _Client(Protocol):
    ...
    # Positional-only (`/`): the scanner always calls these positionally
    # (`fetch_workflows(repo)`), and LocalClient marks the slot `_repo` to
    # signal it is unused. Positional-only frees the implementer from matching
    # the parameter name, so LocalClient conforms without a spurious rename.
