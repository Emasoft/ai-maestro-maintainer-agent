...
def test_persona_states_the_kanban_has_exactly_17_columns(persona: str) -> None:
    ...
    assert re.search(r"(?i)17[- ]column|exactly\s+\*{0,2}17\*{0,2}\s+column", persona), (
        "the persona mentions 17 but not as the column count — a bare number is "
        "not a contract a reader can act on"
    )
...
def test_persona_states_the_board_must_drain(persona: str) -> None:
    ...
    assert re.search(r"(?i)\bdrain\b", persona), (
        "the persona never says the pipeline must drain — without it the 17 "
        "columns are a filing cabinet"
    )
...
def test_persona_knows_the_seeded_aimaestro_rules_are_read_only(persona: str) -> None:
    ...
    assert re.search(r"(?i)restore[sd]?\s+(them\s+)?if\s+edited", persona), (
        "the persona never says the server restores an edited overlay"
    )
...
def test_persona_gives_a_precedence_tie_break_for_the_overlays(persona: str) -> None:
    ...
    assert re.search(r"(?i)report the contradiction", persona), (
        "the persona resolves a contradiction without escalating it — a silent "
        "divergence is a fleet inconsistency nobody can grep for"
    )
...
def test_persona_gates_on_the_overlay_text_not_a_version(persona: str) -> None:
    ...
    """
    assert "grep -q min-approval-requirement" in persona, (
        "the persona shows no concrete capability probe against the seeded overlay"
    )
    assert re.search(r"(?i)never\s+(gate\s+)?on\s+a\s+(branch\s+or\s+)?version", persona), (
        "the persona does not forbid version-gating"
    )
...
def test_persona_treats_the_numeric_tier_field_as_decode_only(persona: str) -> None:
    ...
    assert re.search(r"(?i)approval-tier.{0,120}?(decode-only|decode only|never written)",
                     persona, re.DOTALL), (
        "the persona does not mark approval-tier as decode-only"
    )
    assert "maestro" in persona and re.search(r"(?i)never write", persona), (
        "the deprecated `maestro` read-alias is not marked never-write"
    )
...
def test_persona_declares_manager_and_human_as_its_only_direct_edges(persona: str) -> None:
    ...
    assert re.search(r"(?i)no\s+cos|have\s+no\s+chief-of-staff|NO\s+CHIEF-OF-STAFF", persona), (
        "the persona does not state that MAINTAINER has no CHIEF-OF-STAFF"
    )
    assert re.search(r"(?i)sub-?agents.{0,80}?cannot send", persona, re.DOTALL), (
        "the persona does not state that spawned subagents have no AMP identity"
    )
...
def test_persona_requires_draining_the_amp_inbox_on_every_wake(persona: str) -> None:
    ...
    """
    assert re.search(r"(?i)drain the inbox on EVERY wake", persona), (
        "inbox drain is not scoped to every wake"
    )
...
def test_persona_forbids_direct_api_calls_with_no_escape_hatch(persona: str) -> None:
    ...
    assert "DECOUPLE-BLOCKED" in persona and re.search(r"(?i)revoked", persona), (
        "the revoked escape hatch is not recorded as revoked"
    )
    assert re.search(r"(?i)gh|package-registry", persona), (
        "the rule does not carve out gh / package-registry APIs, so a reader "
        "would over-apply it and stop using gh"
    )
...
def test_persona_mandates_the_self_id_line_verbatim(persona: str) -> None:
    ...
    assert (
        ...
        in persona
    ), "the self-id line is absent or reworded — it must be byte-exact"
